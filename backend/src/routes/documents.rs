...
use crate::{
    error::AppError,
    models::{AuditEvent, StoredDocumentRecord, UploadResponse},
    state::AppState,
};
...
mod tests {
    use super::{
        decode_sha256_hex, sanitize_filename, token_matches, upload_audit_detail, validate_sha256_hex,
        validated_filename, MAX_FILENAME_CHARS,
    };
    ...
    fn audit_detail_mentions_encryption_key_for_encrypted_uploads() {
        let detail = upload_audit_detail(
            "certificate.pdf",
            "bafybeiexample",
            Some("demo-student-rsa-oaep-2026-03-30"),
            true,
        );
        assert!(detail.contains("encrypted"));
        assert!(detail.contains("demo-student-rsa-oaep-2026-03-30"));
    }
}
